...
Produce typed, auditable answers with citations. Use DSPy to optimize at least one component.
...
## Gross Margin
GM = SUM((UnitPrice * Quantity * (1 - Discount)) - (CostOfGoods * Quantity * (1 - Discount)))
...
│   ├── rag/retrieval.py        # TF-IDF or simple retriever (chunking + search)
...
> **Note:** You may assume `CostOfGoods ≈ 0.7 * UnitPrice` if no cost field exists in Northwind; document any variant in your README.
...
```
  ...
  * **`final_answer`**: must match the input `format_hint` exactly (e.g., int, float, object, or list of objects). Floats graded with $\pm 0.01$ tolerance.
...
## Acceptance Criteria & Scoring
  ...
  * **Correctness (40%)**: values match expected ($\pm 0.01$ for floats), and types match `format_hint`.
...
## Notes & Constraints
  ...
  * Keep prompts compact ($\le 1k$ tokens total is plenty).
  * Bound repair to $\le 2$ iterations.
